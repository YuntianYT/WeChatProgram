...
Page({
  ...
  getInitData() {
    ...
    if (this.data.result.method == 1) {
      wx.request({
        url: config.host + 'appSubmitLecDetails/queryEvalDetail',
        data: {
          submit_detail_id: that.data.result.id,
        },
        success(res) {
          that.setData({
            lecDetail: res.data
    ...
    if (this.data.result.method == 2) {
      wx.request({
        url: config.host + 'appSubmitMatrixDetails/queryEvalDetail',
        data: {
          submit_detail_id: that.data.result.id,
        },
        success(res) {
          that.setData({
            matrixDetail: res.data
          })
        }
      })
    }
